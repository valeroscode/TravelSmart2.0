...
A platform that enables users to find the best places in multiple cities here and abroad while planning their trips. This README dives into Travel Smart from a technical perspective. It also summerizes Travel Smart's major features but does not account for every single detail.
To get all the details, please visit Travel Smart at https://travelsmart2-0.onrender.com/

## Technology Used

REACT | JS | FIREBASE | GOlang
...
**1c. Backend:** Travel Smart's backend is written in GO for a few reasons but it mainly boils down to speed. Since Travel Smart at it's full scale would be a data heavy site that stores thousands of bits of information about places and users, the backend needs to be able to get that information and perform operations quickly. In it's current version, Travel Smart uses firebase which comes with it's own backend. All of the data will however, be moved to another database and Go will be used to construct endpoints and perform operations on the database. This is being done for cost savings and in order to have more flexability when writing to the databse.
